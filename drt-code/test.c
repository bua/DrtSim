...
fileRead(void) { 
	...
	if (file != NULL) {
		...
		while (fgets(line, sizeof line, file) != NULL) /* read a line */
		{
			...
			c = radGen();
			if (c == 0){
			//printf(" Calling LTSU As Value is = %d \n ", c);
			...
			{
			//printf(" Calling DTSU As Value is = %d \n ", c);
			dtsuCounter =  dffttable (" Used ");
			dtsuCounter =  ptqTable (ip, sc, " Used ");
			}	
		...
		// Calling DTSU Randomly 
 		calldtsuPrint(dtsuCounter, " DFFT ");
		fflush(stdout);
 		calldtsuPrint(dtsuCounter, " PTQ ");
		fflush(stdout);
	} else {
		perror(filename);
	}
